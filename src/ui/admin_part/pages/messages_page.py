...
class MessagesPage(BasePage):
    ...
    def _find_row(self, name: str) -> Locator:
        ...
        raise AssertionError(f"Message with name {name} was not found.")
    ...
    def get_all_messages(self) -> list[MessageRow]:
        ...
        for i in range(self.message_rows.count()):
            ...
            messages.append(MessageRow(name=values[0], subject=values[1], is_read=is_read))
        return messages
    ...
    def get_message(self, name: str) -> MessageRow:
        ...
        return MessageRow(name=values[0], subject=values[1], is_read=is_read)
    ...
    def wait_at_least_one_message(self, timeout: int = 1000) -> None:
        expect(self.message_rows.first).to_be_visible(timeout=timeout)
